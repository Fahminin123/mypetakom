...
$mem_query = "SELECT MemStatus FROM memberapplication WHERE StudentID = ? ORDER BY VerifiedAt DESC, MemberApplicationID DESC LIMIT 1";
$mem_stmt = $conn->prepare($mem_query);
$mem_stmt->bind_param("s", $student_id);
$mem_stmt->execute();
$mem_result = $mem_stmt->get_result();
$membership_status = "N/A";
if ($mem_result->num_rows > 0) {
    $membership = $mem_result->fetch_assoc();
    $membership_status = $membership['MemStatus'];
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_profile'])) {
    ...
    $update_stmt = $conn->prepare("UPDATE student SET StudentEmail = ?, StudentContact = ? WHERE StudentID = ?");
    $update_stmt->bind_param("sis", $new_email, $new_phone, $student_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload_pic'])) {
    if (isset($_FILES['student_pic']) && $_FILES['student_pic']['error'] === UPLOAD_ERR_OK) {
        $fileTmpPath = $_FILES['student_pic']['tmp_name'];
        $fileType = mime_content_type($fileTmpPath);

        // Only allow image types
        if (strpos($fileType, 'image/') === 0) {
            $imgData = file_get_contents($fileTmpPath);
            $update_pic_stmt = $conn->prepare("UPDATE student SET StudentPic = ? WHERE StudentID = ?");
            $update_pic_stmt->bind_param("bs", $imgData, $student_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_pic'])) {
    $delete_pic_stmt = $conn->prepare("UPDATE student SET StudentPic = NULL WHERE StudentID = ?");
    $delete_pic_stmt->bind_param("s", $student_id);
...
<head>
    ...
    <style>
       ...
       .header { background-color:rgb(31, 8, 77); display: flex; justify-content: space-between; align-items: center; padding: 0 20px; position: fixed; width: 100%; box-sizing: border-box; height: 120px; }
       ...
       .sidebar { width: 200px; background-color: #5e3c99;; color: white; position: fixed; top: 120px; left: 0; bottom: 0; padding: 20px 0; box-sizing: border-box; transition: transform 0.3s ease; }
       ...
       .footer { background-color: #3a1d6e; color: white; }
       ...
       .thumb { width: 180px; height: 180px; object-fit: cover; border: 2px solid #ccc; border-radius: 8px; background: #e0e0e0; }
       ...
       .edit-btn { background-color: #7e57c2; color: white; }
       .edit-btn:hover { background-color: #5e3c99; }
       ...
       .add-pic-btn, .edit-pic-btn { margin-top: 10px; padding: 6px 16px; border-radius: 4px; border: none; font-size: 1rem; cursor: pointer; background-color: #5e3c99; color: white; }
       .add-pic-btn:hover, .edit-pic-btn:hover { background-color: #7e57c2; }
...
<body>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <ul class="menuitems">
            <li><a href="StudentDashboard.php" class="menuitem"><span>Dashboard</span></a></li>
            <li><a href="StudEvent.php" class="menuitem"><span>Event</span></a></li>
            <li><a href="Membership.php" class="menuitem"><span>Membership</span></a></li>
            <li><a href="MeritAwarded.php" class="menuitem"><span>Merit Awarded</span></a></li>
            <li><a href="MissingMerit.php" class="menuitem"><span>Missing Merit</span></a></li>
        </ul>
    </nav>
    ...
    <div class="maincontent" id="maincontent">
        ...
        <div class="seccontent">
            ...
            <?php endif; ?>
            <div class="card-top">
                <?php if (!empty($student['StudentPic'])): ?>
                    <img class="thumb" src="view_student_pic.php?id=<?php echo urlencode($student['StudentID']); ?>" alt="Student Picture" />
                    <!-- Edit and delete picture form/buttons -->
                    ...
                    <form method="POST" action="" enctype="multipart/form-data" id="editPicForm" class="pic-upload-form hidden">
                        <input type="file" name="student_pic" accept="image/*" required style="margin-bottom:8px;">
                        <button type="submit" name="upload_pic" class="add-pic-btn">Upload</button>
                        <div class="pic-upload-buttons">
                            <button type="button" class="cancel-btn" id="cancelEditPicBtn" style="margin-top:8px;">Cancel</button>
                            <button type="submit" name="delete_pic" class="del-pic-btn" style="margin-top:8px;" onclick="return confirm('Are you sure you want to delete your profile picture?');">Delete Picture</button>
                        </div>
                    </form>
                <?php else: ?>
                    ...
                    <form method="POST" action="" enctype="multipart/form-data" id="addPicForm" class="pic-upload-form hidden">
                        ...
                        <button type="submit" name="upload_pic" class="add-pic-btn">Upload</button>
                        <div class="pic-upload-buttons">
                            <button type="button" class="cancel-btn" id="cancelAddPicBtn" style="margin-top:8px;">Cancel</button>
                        </div>
                    </form>
                <?php endif; ?>
            </div>
            <form id="profileForm" method="POST" action="">
                <div class="info-row">
                    ...
                    <div><?php echo htmlspecialchars($student['StudentName']); ?></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Student ID:</div>
                    <div><?php echo htmlspecialchars($student['StudentID']); ?></div>
                </div>
                <div class="info-row">
                    <div class="info-label">Email:</div>
                    <div id="email-view"><?php echo htmlspecialchars($student['StudentEmail']); ?></div>
                    <input type="email" id="email-edit" name="email" class="input-edit" value="<?php echo htmlspecialchars($student['StudentEmail']); ?>" style="display:none;" required>
                </div>
                <div class="info-row">
                    <div class="info-label">Phone Number:</div>
                    <div id="phone-view"><?php echo htmlspecialchars($student['StudentContact']); ?></div>
                    <input type="text" id="phone-edit" name="phone" class="input-edit" value="<?php echo htmlspecialchars($student['StudentContact']); ?>" style="display:none;" pattern="[0-9]{10,13}" title="Please enter a valid phone number">
                </div>
                <div class="info-row">
                    <div class="info-label">Membership Status:</div>
                    <div><?php echo htmlspecialchars($membership_status); ?></div>
                </div>
