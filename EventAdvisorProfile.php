...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_profile'])) {
    ...
    $update_stmt->bind_param("sis", $new_email, $new_phone, $staff_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload_pic'])) {
    if (isset($_FILES['staff_pic']) && $_FILES['staff_pic']['error'] === UPLOAD_ERR_OK) {
        ...
        if (strpos($fileType, 'image/') === 0) {
            ...
            $update_pic_stmt->bind_param("bs", $imgData, $staff_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_pic'])) {
    ...
    $delete_pic_stmt->bind_param("s", $staff_id);
...
<head>
    ...
    <style>
        ...
        .menuitem:hover { background-color: #1f2d3d; }
        .menuitem.active { background-color: #1abc9c; font-weight: 500; }
        .togglebutton { background-color: #1f2d3d; color: white; border: 1px solid #2c3e50; padding: 8px 12px; border-radius: 4px; cursor: pointer; font-size: 1rem; display: flex; align-items: center; gap: 8px; transition: background 0.2s; }
        .togglebutton:hover { background-color: #2c3e50; }
        ...
        .footer { background-color: #1f2d3d; color: white; }
        ...
        .edit-btn { background-color: #1f2d3d; color: white; border: 1px solid #2c3e50; transition: background 0.2s;}
        .edit-btn:hover { background-color: #2c3e50; }
        ...
        .add-pic-btn, .edit-pic-btn { margin-top: 10px; padding: 6px 16px; border-radius: 4px; border: none; font-size: 1rem; cursor: pointer; background-color: #1f2d3d; color: white; }
        .add-pic-btn:hover, .edit-pic-btn:hover { background-color: #2c3e50; }
...
<body>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <ul class="menuitems">
            <li><a href="EventAdvisorDashboard.php" class="menuitem"><span>Dashboard</span></a></li>
        ...
        <div class="seccontent">
            ...
            <div class="card-top">
                <?php if (!empty($staff['StaffPic'])): ?>
                    <img class="thumb" src="view_staff_pic.php?id=<?php echo urlencode($staff['StaffID']); ?>" alt="Staff Picture" />
